...
if ($report) {
    ...
    $sql_violation = "SELECT * FROM violation WHERE ticket_number = ?";
    $stmt_violation = $conn->prepare($sql_violation);
    $stmt_violation->bind_param("s", $ticket_number);
    $stmt_violation->execute();
    $violation_count = $stmt_violation->get_result()->num_rows;

    $sql_violation2 = "SELECT * FROM 2_violation WHERE ticket_number = ?";
    $stmt_violation2 = $conn->prepare($sql_violation2);
    $stmt_violation2->bind_param("s", $ticket_number);
    $stmt_violation2->execute();
    $violation_count2 = $stmt_violation2->get_result()->num_rows;

    $sql_violation3 = "SELECT * FROM 3_violation WHERE ticket_number = ?";
    $stmt_violation3 = $conn->prepare($sql_violation3);
    $stmt_violation3->bind_param("s", $ticket_number);
